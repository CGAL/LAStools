...
// these fields describe the point format LAS specific
  ...
  las_BOOL have_gps_time;
  las_BOOL have_rgb;
  las_BOOL have_nir;
  las_BOOL have_wavepacket;
...
// these functions set the desired point format (and maybe add on attributes in extra bytes)
  ...
  las_BOOL init(const LASquantizer* quantizer, const U8 point_type, const U16 point_size, const LASattributer* attributer=0)
  {
    // clean the point
  ...
  las_BOOL init(const LASquantizer* quantizer, const U32 num_items, const LASitem* items, const LASattributer* attributer=0)
  {
    U32 i;
  ...
  las_BOOL inside_rectangle(const F64 r_min_x, const F64 r_min_y, const F64 r_max_x, const F64 r_max_y) const
  {
    F64 xy;
    xy = get_x();
  ...
  las_BOOL inside_tile(const F32 ll_x, const F32 ll_y, const F32 ur_x, const F32 ur_y) const
  {
    F64 xy;
    xy = get_x();
  ...
  las_BOOL inside_circle(const F64 center_x, const F64 center_y, F64 squared_radius) const
  {
  ...
  las_BOOL inside_box(const F64 min_x, const F64 min_y, const F64 min_z, const F64 max_x, const F64 max_y, const F64 max_z) const
  {
    F64 xyz;
    xyz = get_x();
  ...
  las_BOOL inside_bounding_box(const F64 min_x, const F64 min_y, const F64 min_z, const F64 max_x, const F64 max_y, const F64 max_z) const
  {
    F64 xyz;
    xyz = get_x();
    if (xyz < min_x || xyz > max_x) return FALSE;
  ...
  inline las_BOOL is_first() const { return get_return_number() <= 1; };
  inline las_BOOL is_intermediate() const { return (!is_first() && !is_last()); };
  inline las_BOOL is_last() const { return get_return_number() >= get_number_of_returns(); };
  inline las_BOOL is_single() const { return get_number_of_returns() <= 1; };

  inline las_BOOL is_first_of_many() const { return !is_single() && is_first(); };
  inline las_BOOL is_last_of_many() const { return !is_single() && is_last(); };
  ...
  inline las_BOOL has_attribute(I32 index) const
  {
    if (attributer)
    {
  ...
  inline las_BOOL get_attribute(I32 index, U8* data) const
  {
  ...
  inline las_BOOL set_attribute(I32 index, const U8* data) 
  {
    if (has_attribute(index))
    {
