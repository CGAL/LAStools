...
public:
  ...
  las_BOOL write(const U8* item);
  las_BOOL chunk_sizes();
  las_BOOL chunk_bytes();
...
private:
  ...
  las_BOOL createAndInitModelsAndCompressors(U32 context, const U8* item);
...
public:
  ...
  las_BOOL init(const U8* item);
  las_BOOL write(const U8* item); //, U32 current_context=0);
  las_BOOL chunk_sizes();
  las_BOOL chunk_bytes();
