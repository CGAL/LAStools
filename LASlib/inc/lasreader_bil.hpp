...
public:
  ...
  virtual las_BOOL open(const CHAR* file_name);
...
private:
  ...
  F32 xdim, ydim;
  F32 nodata;
  las_BOOL floatpixels;
  las_BOOL signedpixels;

  void clean();
  las_BOOL read_hdr_file(const CHAR* file_name);
  las_BOOL read_blw_file(const CHAR* file_name);
...
{
public:
  las_BOOL open(const CHAR* file_name);
