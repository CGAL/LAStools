...
las_BOOL LASreaderDTM::open(const CHAR* file_name)
{
  if (file_name == 0)
  {
...
las_BOOL LASreaderDTM::seek(const I64 p_index)
{
  return FALSE;
}

las_BOOL LASreaderDTM::read_point_default()
{
...
void LASreaderDTM::close(las_BOOL close_stream)
{
  if (file)
  {
    fclose(file);
...
las_BOOL LASreaderDTM::reopen(const CHAR* file_name)
{
  if (file_name == 0)
  {
    fprintf(stderr,"ERROR: file name pointer is zero\n");
...
las_BOOL LASreaderDTMrescale::open(const CHAR* file_name)
{
...
las_BOOL LASreaderDTMreoffset::open(const CHAR* file_name)
{
...
las_BOOL LASreaderDTMrescalereoffset::open(const CHAR* file_name)
{
  LASreaderDTM::set_scale_factor(scale_factor);
  LASreaderDTM::set_offset(offset);
  if (!LASreaderDTM::open(file_name)) return FALSE;
