...
las_BOOL LASinventory::init(const LASheader* header)
{
  if (header)
  {
    U32 i;
...
las_BOOL LASinventory::add(const LASpoint* point)
{
...
las_BOOL LASinventory::update_header(LASheader* header) const
{
  if (header)
  {
    int i;
...
las_BOOL LASsummary::add(const LASpoint* point)
{
...
las_BOOL LAShistogram::parse(int argc, char* argv[])
{
  int i;
  for (i = 1; i < argc; i++)
  {
...
las_BOOL LAShistogram::histo(const CHAR* name, F32 step)
{
  if (strcmp(name, "x") == 0)
...
las_BOOL LAShistogram::histo_avg(const CHAR* name, F32 step, const CHAR* name_avg)
{
...
las_BOOL LASoccupancyGrid::add(const LASpoint* point)
{
  I32 pos_x, pos_y;
  if (grid_spacing < 0)
  {
...
las_BOOL LASoccupancyGrid::add(I32 pos_x, I32 pos_y)
{
  if (grid_spacing < 0)
  {
    grid_spacing = -grid_spacing;
...
las_BOOL LASoccupancyGrid::add_internal(I32 pos_x, I32 pos_y)
{
  pos_y = pos_y - anker;
...
las_BOOL LASoccupancyGrid::occupied(const LASpoint* point) const
{
...
las_BOOL LASoccupancyGrid::occupied(I32 pos_x, I32 pos_y) const
{
  if (grid_spacing < 0)
  {
    return FALSE;
...
las_BOOL LASoccupancyGrid::active() const
{
...
las_BOOL LASoccupancyGrid::write_asc_grid(const CHAR* file_name) const
{
