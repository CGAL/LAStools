...
las_BOOL LASreaderQFIT::open(const char* file_name)
{
  if (file_name == 0)
  {
...
las_BOOL LASreaderQFIT::open(ByteStreamIn* stream)
{
  U32 i;
...
las_BOOL LASreaderQFIT::seek(const I64 p_index)
{
  if (p_index < npoints)
  {
    p_count = p_index;
...
las_BOOL LASreaderQFIT::read_point_default()
{
  if (p_count < npoints)
  {
    try { stream->getBytes((U8*)buffer, version); } catch(...)
...
void LASreaderQFIT::close(las_BOOL close_stream)
{
  if (close_stream)
  {
    if (stream)
...
las_BOOL LASreaderQFIT::reopen(const char* file_name)
{
  if (file_name == 0)
  {
    fprintf(stderr,"ERROR: file name pointer is zero\n");
...
las_BOOL LASreaderQFITrescale::open(ByteStreamIn* stream)
{
...
las_BOOL LASreaderQFITreoffset::open(ByteStreamIn* stream)
{
  if (!LASreaderQFIT::open(stream)) return FALSE;
  // do we need to change anything
...
las_BOOL LASreaderQFITrescalereoffset::open(ByteStreamIn* stream)
{
  if (!LASreaderQFIT::open(stream)) return FALSE;
  // do we need to change anything
  if (scale_factor[0] && (header.x_scale_factor != scale_factor[0]))
