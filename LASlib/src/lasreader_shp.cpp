...
las_BOOL LASreaderSHP::open(const char* file_name)
{
  if (file_name == 0)
  {
...
las_BOOL LASreaderSHP::seek(const I64 p_index)
{
  return FALSE;
}

las_BOOL LASreaderSHP::read_point_default()
{
...
void LASreaderSHP::close(las_BOOL close_stream)
{
  if (file)
  {
...
las_BOOL LASreaderSHP::reopen(const char* file_name)
{
  if (file_name == 0)
  {
    fprintf(stderr,"ERROR: file name pointer is zero\n");
...
las_BOOL LASreaderSHPrescale::open(const char* file_name)
{
...
las_BOOL LASreaderSHPreoffset::open(const char* file_name)
{
  if (!LASreaderSHP::open(file_name)) return FALSE;
  // do we need to change anything
...
las_BOOL LASreaderSHPrescalereoffset::open(const char* file_name)
{
  if (!LASreaderSHP::open(file_name)) return FALSE;
  // do we need to change anything
  if (scale_factor[0] && (header.x_scale_factor != scale_factor[0]))
