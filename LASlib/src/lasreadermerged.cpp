...
las_BOOL LASreaderMerged::add_file_name(const char* file_name)
{
...
void LASreaderMerged::set_files_are_flightlines(las_BOOL files_are_flightlines)
{
...
void LASreaderMerged::set_apply_file_source_ID(las_BOOL apply_file_source_ID)
{
...
void LASreaderMerged::set_populate_header(las_BOOL populate_header)
{
  this->populate_header = populate_header;
}

void LASreaderMerged::set_keep_lastiling(las_BOOL keep_lastiling)
{
  this->keep_lastiling = keep_lastiling;
}

las_BOOL LASreaderMerged::open()
{
...
las_BOOL LASreaderMerged::inside_tile(const F32 ll_x, const F32 ll_y, const F32 size)
{
  inside = 1;
  t_ll_x = ll_x;
  t_ll_y = ll_y;
...
las_BOOL LASreaderMerged::inside_circle(const F64 center_x, const F64 center_y, const F64 radius)
{
  inside = 2;
  c_center_x = center_x;
  c_center_y = center_y;
...
las_BOOL LASreaderMerged::inside_rectangle(const F64 min_x, const F64 min_y, const F64 max_x, const F64 max_y)
{
  inside = 3;
  r_min_x = min_x;
  r_min_y = min_y;
...
las_BOOL LASreaderMerged::read_point_default()
{
...
void LASreaderMerged::close(las_BOOL close_stream)
{
  if (lasreader) 
  {
...
las_BOOL LASreaderMerged::reopen()
{
  p_count = 0;
  file_name_current = 0;
...
las_BOOL LASreaderMerged::open_next_file()
{
