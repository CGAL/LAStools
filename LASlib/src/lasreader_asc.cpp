...
las_BOOL LASreaderASC::open(const CHAR* file_name, las_BOOL comma_not_point)
{
  if (file_name == 0)
  {
...
las_BOOL LASreaderASC::seek(const I64 p_index)
{
  return FALSE;
}

las_BOOL LASreaderASC::read_point_default()
{
  F32 elevation;
...
void LASreaderASC::close(las_BOOL close_stream)
{
  if (file)
  {
...
las_BOOL LASreaderASC::reopen(const CHAR* file_name)
{
  if (file_name == 0)
  {
    fprintf(stderr,"ERROR: file name pointer is zero\n");
...
las_BOOL LASreaderASCrescale::open(const CHAR* file_name, las_BOOL comma_not_point)
{
...
las_BOOL LASreaderASCreoffset::open(const CHAR* file_name, las_BOOL comma_not_point)
{
...
las_BOOL LASreaderASCrescalereoffset::open(const CHAR* file_name, las_BOOL comma_not_point)
{
  LASreaderASC::set_scale_factor(scale_factor);
  LASreaderASC::set_offset(offset);
  if (!LASreaderASC::open(file_name, comma_not_point)) return FALSE;
