...
las_BOOL LASreader::inside_none()
{
  if (filter || transform)
  {
    read_complex = &LASreader::read_point_default;
...
las_BOOL LASreader::inside_tile(const F32 ll_x, const F32 ll_y, const F32 size)
{
  inside = 1;
  t_ll_x = ll_x;
  t_ll_y = ll_y;
...
las_BOOL LASreader::inside_circle(const F64 center_x, const F64 center_y, const F64 radius)
{
  inside = 2;
  c_center_x = center_x;
  c_center_y = center_y;
...
las_BOOL LASreader::inside_rectangle(const F64 min_x, const F64 min_y, const F64 max_x, const F64 max_y)
{
  inside = 3;
  r_min_x = min_x;
  r_min_y = min_y;
...
las_BOOL LASreader::read_point_inside_tile()
{
...
las_BOOL LASreader::read_point_inside_tile_indexed()
{
...
las_BOOL LASreader::read_point_inside_circle()
{
  while (read_point_default())
  {
...
las_BOOL LASreader::read_point_inside_circle_indexed()
{
  while (index->seek_next((LASreader*)this))
  {
...
las_BOOL LASreader::read_point_inside_rectangle()
{
  while (read_point_default())
  {
...
las_BOOL LASreader::read_point_inside_rectangle_indexed()
{
  while (index->seek_next((LASreader*)this))
  {
...
las_BOOL LASreader::read_point_none()
{
  return FALSE;
}

las_BOOL LASreader::read_point_filtered()
{
...
las_BOOL LASreader::read_point_transformed()
{
...
las_BOOL LASreader::read_point_filtered_and_transformed()
{
...
las_BOOL LASreadOpener::is_piped() const
{
  return (!file_names && use_stdin);
}

las_BOOL LASreadOpener::is_inside() const
{
...
las_BOOL LASreadOpener::is_buffered() const
{
  return ((buffer_size > 0) && ((file_name_number > 1) || (neighbor_file_name_number > 0)));
}

las_BOOL LASreadOpener::is_header_populated() const
{
...
LASreader* LASreadOpener::open(const CHAR* other_file_name, las_BOOL reset_after_other)
{
...
las_BOOL LASreadOpener::reopen(LASreader* lasreader, las_BOOL remain_buffered)
{
  if (lasreader == 0)
  {
...
las_BOOL LASreadOpener::parse(int argc, char* argv[])
{
  int i;
  for (i = 1; i < argc; i++)
  {
...
void LASreadOpener::set_merged(const las_BOOL merged)
{
  this->merged = merged;
}
...
void LASreadOpener::set_auto_reoffset(const las_BOOL auto_reoffset)
{
  this->auto_reoffset = auto_reoffset;
}

void LASreadOpener::set_files_are_flightlines(const las_BOOL files_are_flightlines)
{
  this->files_are_flightlines = files_are_flightlines;
}

void LASreadOpener::set_apply_file_source_ID(const las_BOOL apply_file_source_ID)
{
...
void LASreadOpener::set_file_name(const CHAR* file_name, las_BOOL unique)
{
...
#ifdef _WIN32
#include <windows.h>
las_BOOL LASreadOpener::add_file_name(const CHAR* file_name, las_BOOL unique)
{
  las_BOOL r = FALSE;
  HANDLE h;
...
#ifdef _WIN32
las_BOOL LASreadOpener::add_file_name_single(const CHAR* file_name, las_BOOL unique)
#else
las_BOOL LASreadOpener::add_file_name(const CHAR* file_name, las_BOOL unique)
#endif
{
...
las_BOOL LASreadOpener::add_list_of_files(const CHAR* list_of_files, las_BOOL unique)
{
...
las_BOOL LASreadOpener::set_file_name_current(U32 file_name_id)
{
  if (file_name_id < file_name_number)
  {
...
#ifdef _WIN32
#include <windows.h>
las_BOOL LASreadOpener::add_neighbor_file_name(const CHAR* neighbor_file_name, las_BOOL unique)
{
  las_BOOL r = FALSE;
  HANDLE h;
  WIN32_FIND_DATA info;
...
#ifdef _WIN32
las_BOOL LASreadOpener::add_neighbor_file_name_single(const CHAR* neighbor_file_name, las_BOOL unique)
#else
las_BOOL LASreadOpener::add_neighbor_file_name(const CHAR* neighbor_file_name, las_BOOL unique)
#endif
{
...
las_BOOL LASreadOpener::set_point_type(U8 point_type)
{
  if (point_type > 10)
  {
    return FALSE;
...
void LASreadOpener::set_populate_header(las_BOOL populate_header)
{
  this->populate_header = populate_header;
}

void LASreadOpener::set_keep_lastiling(las_BOOL keep_lastiling)
{
  this->keep_lastiling = keep_lastiling;
}

void LASreadOpener::set_pipe_on(las_BOOL pipe_on)
{
...
las_BOOL LASreadOpener::active() const
{
