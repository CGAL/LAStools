...
las_BOOL LASreaderBIN::open(const char* file_name)
{
  if (file_name == 0)
  {
...
las_BOOL LASreaderBIN::open(ByteStreamIn* stream)
{
  int i;
...
las_BOOL LASreaderBIN::seek(const I64 p_index)
{
  if (p_index < npoints)
  {
...
las_BOOL LASreaderBIN::read_point_default()
{
  if (p_count < npoints)
  {
    int echo;
...
void LASreaderBIN::close(las_BOOL close_stream)
{
  if (close_stream)
  {
    if (stream)
...
las_BOOL LASreaderBINrescale::open(ByteStreamIn* stream)
{
...
las_BOOL LASreaderBINreoffset::open(ByteStreamIn* stream)
{
  if (!LASreaderBIN::open(stream)) return FALSE;
  // do we need to change anything
...
las_BOOL LASreaderBINrescalereoffset::open(ByteStreamIn* stream)
{
  if (!LASreaderBIN::open(stream)) return FALSE;
  // do we need to change anything
  if (scale_factor[0] && (header.x_scale_factor != scale_factor[0]))
